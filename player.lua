...
function Player:init(args)
  ...
  if self.character == 'locust' then
    ...
    self.t:cooldown(5, function() local enemies = self:get_objects_in_shape(self.attack_sensor, main.current.enemies); return enemies and #enemies > 0 end, function()
      ...
      if closest_enemy then
		self:attack(32, {x = closest_enemy.x, y = closest_enemy.y})
      end
    end, nil, nil, 'attack')

  elseif self.character == 'initiate' then
    self.attack_sensor = Circle(self.x, self.y, 96)
    self.t:cooldown(3, function() local enemies = self:get_objects_in_shape(self.attack_sensor, main.current.enemies); return enemies and #enemies > 0 end, function()
	  ...
	  if (self.level == 3) then
		if not self.init_ritual_dmg_m then
			self.init_ritual_dmg_m = 1
		end
		self.init_ritual_dmg_m = self.init_ritual_dmg_m + 0.01
	  end
	  self:cult_attack()
      if closest_enemy then
		unit_1:hit(((self.dmg*0.5)))
        self:shoot(self:angle_to_object(closest_enemy), {dmg = (self.dmg*3)})
      end
    end, nil, nil, 'shoot')
      ...
      local closest_enemy = self:get_closest_object_in_shape(self.attack_sensor, main.current.enemies)
		if closest_enemy then
			...
			for _, enemy in ipairs(enemies) do
				enemy:hit(self.dmg*(self.distance_dmg_m or 1))
				LightningLine{group = main.current.effects, src = self, dst = enemy}
			end
		end
    end, nil, nil, 'shoot')
  ...
  elseif self.character == 'necromancer' then
      ...
      self.t:every(4, function()
		  ...
		  unit_1:hit(((self.dmg)))
		  self:cult_attack()
		  Critter{group = main.current.main, x = self.x, y = self.y, color = dblue[0], r = random:float(0, 2*math.pi), v = 10, dmg = self.dmg, parent = self, size = 1}
	  end, nil, nil, 'shoot')
  ...
  elseif self.character == 'adventurer' then
  ...
  elseif self.character == 'sylph' then
  ...
  elseif self.character == 'mortar' then
    self.attack_sensor = Circle(self.x, self.y, 128)
    self.t:cooldown(3, function() local enemies = self:get_objects_in_shape(self.attack_sensor, main.current.enemies); return enemies and #enemies > 0 end, function()
	  local closest_enemy = self:get_closest_object_in_shape(self.attack_sensor, main.current.enemies)
	  local posx = closest_enemy.x
	  local posy = closest_enemy.y
	  self.t:after(0.5, function()
			if closest_enemy then
				self:dot_attack(30, {duration = 12, x = closest_enemy.x, y = closest_enemy.y})
			end
	  end)
    end, nil, nil, 'attack')

  elseif self.character == 'brawler' then
    self.attack_sensor = Circle(self.x, self.y, 48)
    self.t:cooldown(2, function() local enemies = self:get_objects_in_shape(self.attack_sensor, main.current.enemies); return enemies and #enemies > 0 end, function()
      local closest_enemy = self:get_closest_object_in_shape(self.attack_sensor, main.current.enemies)
      if closest_enemy then
  ...
  elseif self.character == 'rocketeer' then -- Sniper Hour
    self.attack_sensor = Circle(self.x, self.y, 96)
    self.t:cooldown(2, function() local enemies = self:get_objects_in_shape(self.attack_sensor, main.current.enemies); return enemies and #enemies > 0 end, function()
      local closest_enemy = self:get_closest_object_in_shape(self.attack_sensor, main.current.enemies)
      if closest_enemy then
  ...
  elseif self.character == 'sniper' then -- Sniper Hour
    self.attack_sensor = Circle(self.x, self.y, 256)
    self.t:cooldown(8, function() local enemies = self:get_objects_in_shape(self.attack_sensor, main.current.enemies); return enemies and #enemies > 0 end, function()
      local closest_enemy = self:get_closest_object_in_shape(self.attack_sensor, main.current.enemies)
      if closest_enemy then
        self:shoot(self:angle_to_object(closest_enemy), {pierce = 20, v = 700})
      end
    end, nil, nil, 'shoot')

  elseif self.character == 'vagrant' then
    self.attack_sensor = Circle(self.x, self.y, 96)
    self.t:cooldown(2, function() local enemies = self:get_objects_in_shape(self.attack_sensor, main.current.enemies); return enemies and #enemies > 0 end, function()
      local closest_enemy = self:get_closest_object_in_shape(self.attack_sensor, main.current.enemies)
      if closest_enemy then
  ...
  elseif self.character == 'wizard' then
    self.attack_sensor = Circle(self.x, self.y, 128)
    self.t:cooldown(1.6, function() local enemies = self:get_objects_in_shape(self.attack_sensor, main.current.enemies); return enemies and #enemies > 0 end, function()
      local closest_enemy = self:get_closest_object_in_shape(self.attack_sensor, main.current.enemies)
      if closest_enemy then
  ...
  elseif self.character == 'cleric' then
    ...
    end, nil, nil, 'heal') -- boomerang

  elseif self.character == 'boomerang' then
    self.sorcerer_count = 0
    self.attack_sensor = Circle(self.x, self.y, 128)
    self.t:cooldown(4, function() local enemies = self:get_objects_in_shape(self.attack_sensor, main.current.enemies); return enemies and #enemies > 0 end, function()
      local closest_enemy = self:get_closest_object_in_shape(self.attack_sensor, main.current.enemies)
      if closest_enemy then
        self:shoot(self:angle_to_object(closest_enemy), {pierce = 75, v = 160, ricochet = 10})
		if (self.level == 3) then
        ...
        elseif main.current.sorcerer_level > 0 then
          self.sorcerer_count = self.sorcerer_count + 1
          if self.sorcerer_count >= ((main.current.sorcerer_level == 3 and 2) or (main.current.sorcerer_level == 2 and 3) or (main.current.sorcerer_level == 1 and 4)) then
            self:sorcerer_repeat()
            self.sorcerer_count = 0
  ...
  elseif self.character == 'psychic' then
    ...
    self.t:cooldown(2.5, function() local enemies = self:get_objects_in_shape(self.attack_sensor, main.current.enemies); return enemies and #enemies > 0 end, function()
      local strike = function()
        ...
        if enemy then
          if self.level == 3 then
            self:attack(48, {x = enemy.x, y = enemy.y})
            self.t:after(0.5, function()
              local enemy = self:get_random_object_in_shape(self.attack_sensor, main.current.enemies)
              if enemy then
                self:attack(48, {x = enemy.x, y = enemy.y})
  ...
  elseif self.character == 'vulcanist' then
    ...
    elseif self.character == 'acolyte' then
    self.sorcerer_count = 0
    self.attack_sensor = Circle(self.x, self.y, 128)
    self.t:every(4, function()
      local acolyte = function()
        local enemies = main.current.main:get_objects_by_classes(main.current.enemies)
        local x, y = 0, 0
        if enemies and #enemies > 0 then
          for _, enemy in ipairs(enemies) do
        ...
        main.current.t:every_immediate(0.1, function()
          ...
          local objects = main.current.main:get_objects_in_shape(check_circle, {Player, Seeker, EnemyCritter, Critter, Saboteur, Pet, Turret, Sentry, Bomb})
          if #objects == 0 then
            Shrine{group = main.current.main, x = x, y = y, color = self.color, parent = self, rs = 24, level = self.level}
            main.current.t:cancel('shrine_spawn')
          end
        end, nil, nil, 'shrine_spawn')
      end
	  self:cult_attack()
      acolyte()
	  self:hit((self.dmg))
  ...
  elseif self.character == 'plague_doctor' then
    self.t:every(5, function()
      self:dot_attack(24, {duration = 30, plague_doctor_unmovable = true})
    end, nil, nil, 'attack')
  ...
  elseif self.character == 'lich' then
    ...
    elseif self.character == 'psylander' then
	self.reach = 64
	self.randomy = random:float(0, 3)
    self.attack_sensor = Circle(self.x, self.y, self.reach)
	self.counter = 0
    self.t:after(0.05, function()
      self.dot_area = DotArea{group = main.current.effects, x = self.x, y = self.y, rs = self.area_size_m*48, color = self.color, dmg = self.area_dmg_m*self.dmg, character = self.character, level = self.level, parent = self}
    end)
	if (self.level == 3) then
		self.t:cooldown(1, function()  local enemies = self:get_objects_in_shape(self.attack_sensor, main.current.enemies); return enemies and #enemies > 0 end, function()
			self.attack_sensor2 = Circle(self.x, self.y, self.reach+32)
			local closest_enemy = self:get_closest_object_in_shape(self.attack_sensor2, main.current.enemies)
			self.counter = self.counter + 1
			if (closest_enemy) and (self.psylander_rogue_bonus > 1) then
				self:shoot(self:angle_to_object(closest_enemy), {chain = (self.psylander_rogue_bonus-1)*3})
			end
			if (self.counter%5 > 4) and (self.psylander_nuker_bonus > 1) then
				self:attack((self.psylander_nuker_bonus*3)+15, {x = closest_enemy.x, y = closest_enemy.y})
			end
		end, nil, nil, 'attack')
		self.t:every(0.01, function()
			if (self.psylander_ranger_bonus) then
				self.reach = (self.psylander_ranger_bonus*16)+48
			else
				self.reach = 64
			end
			Circle(self.x, self.y, self.reach)
		end)
    end
  ...
  elseif self.character == 'silencer' then
    ...
    self.t:cooldown(6, function() local enemies = self:get_objects_in_shape(self.attack_sensor, main.current.enemies); return enemies and #enemies > 0 end, function()
      local curse = function()
        buff1:play{pitch = random:float(0.9, 1.1), volume = 0.5}
        local enemies = table.first2(table.shuffle(self:get_objects_in_shape(self.wide_attack_sensor, main.current.enemies)),
          6 + ((self.malediction == 1 and 1) or (self.malediction == 2 and 3) or (self.malediction == 3 and 5) or 0) + ((main.current.curser_level == 2 and 3) or (main.current.curser_level == 1 and 1) or 0) + ((self.level == 3 and 3) or 0))
        for _, enemy in ipairs(enemies) do
  ...
  elseif self.character == 'assassin' then
    self.attack_sensor = Circle(self.x, self.y, 60)
    self.t:cooldown(1.8, function() local enemies = self:get_objects_in_shape(self.attack_sensor, main.current.enemies); return enemies and #enemies > 0 end, function()
      local closest_enemy = self:get_closest_object_in_shape(self.attack_sensor, main.current.enemies)
      if closest_enemy then
      ...
      if self.barrager_counter == 3 and self.level == 3 then
        ...
        for i = 1, 15 do
          self.t:after((i-1)*0.05, function()
            self:shoot(r + random:float(-math.pi/32, math.pi/32), {knockback = (self.level == 3 and 18 or 9)})
          end)
        end
      else
        for i = 1, 3 do
  ...
  elseif self.character == 'infestor' then
    ...
    self.t:cooldown(6, function() local enemies = self:get_objects_in_shape(self.attack_sensor, main.current.enemies); return enemies and #enemies > 0 end, function()
      buff1:play{pitch = random:float(0.9, 1.1), volume = 0.5}
      local enemies = table.first2(table.shuffle(self:get_objects_in_shape(self.wide_attack_sensor, main.current.enemies)),
        8 + ((self.malediction == 1 and 1) or (self.malediction == 2 and 3) or (self.malediction == 3 and 5) or 0) + ((main.current.curser_level == 2 and 3) or (main.current.curser_level == 1 and 1) or 0) + (self.level == 3 and 2) or 0)
      for _, enemy in ipairs(enemies) do
  ...
  elseif self.character == 'flagellant' then
    self.t:every(8, function()
      ...
      for _, unit in ipairs(all_units) do
        ...
        if self.level == 3 then
          unit.flagellant_dmg_m = unit.flagellant_dmg_m + 0.15
        else
          unit.flagellant_dmg_m = unit.flagellant_dmg_m + 0.05
        end
      end
    end, nil, nil, 'buff')
     elseif (self.character == 'psylander') then
	self:after(0.1, function()
		self.psylander_warrior_def_m = 1
		self.psylander_enchanter_dmg_m = 1
		self.psylander_rogue_mvspd_m = 1
		self.psylander_nuker_area_dmg_m = 1
		self.psylander_nuker_area_size_m = 1
		self.psylander_orb_rotate_speed = 1
		self.psylander_orb_rotate_speed = 1
		self.psylander_forcer_power = 1
		self.psylander_ranger_bonus = 1
		self.psylander_nuker_bonus = 1
		self.psylander_rogue_bonus = 1
		local all_units = self:get_all_units()
		for _, unit in ipairs(all_units) do
			  if self.level == 3 and table.any(unit.classes, function(v) return v == 'warrior' end) and unit.character ~= 'psylander' then
				if (self.executions < 6) then
						self.executions = self.executions + 1
						self.psylander_warrior_def_m = self.psylander_warrior_def_m + 0.75
						if table.any(unit.classes, function(v) return v == 'enchanter' end) then
							self.psylander_enchanter_dmg_m = self.psylander_enchanter_dmg_m + 0.25
						end
						if table.any(unit.classes, function(v) return v == 'rogue' end) then
							self.psylander_rogue_mvspd_m = self.psylander_rogue_mvspd_m + 0.25
							self.psylander_rogue_bonus = self.psylander_rogue_bonus + 1
						end
						if table.any(unit.classes, function(v) return v == 'nuker' end) then
							self.psylander_nuker_area_dmg_m = self.psylander_nuker_area_size_m + 0.05
							self.psylander_nuker_area_dmg_m = self.psylander_nuker_area_dmg_m + 0.25
						end
						if table.any(unit.classes, function(v) return v == 'forcer' end) then
							self.knockback_m = self.knockback_m + 0.5
						end
						if table.any(unit.classes, function(v) return v == 'explorer' end) then
							self.psylander_orb_rotate_speed = self.psylander_orb_rotate_speed + 0.5
							self.psylander_enchanter_dmg_m = self.psylander_enchanter_dmg_m + 0.15
						end
						...
						if table.any(unit.classes, function(v) return v == 'ranger' end) then
							self.psylander_ranger_bonus = self.psylander_ranger_bonus + 1
						end
					end
				end
			end
  ...
  if self.ouroboros_technique_r then
    self.t:after(0.01, function()
      self.t:every((self.ouroboros_technique_r == 1 and 0.2) or (self.ouroboros_technique_r == 2 and 0.166) or (self.ouroboros_technique_r == 3 and 0.144), function()
  ...
  if self.centipede then self.centipede_mvspd_m = (self.centipede == 1 and 1.15) or (self.centipede == 2 and 1.25) or (self.centipede == 3 and 1.35) end
  if self.amplify then self.amplify_area_dmg_m = (self.amplify == 1 and 1.2) or (self.amplify == 2 and 1.4) or (self.amplify == 3 and 1.6) end

  if self.ballista and launches_projectiles(self.character) then
    self.ballista_dmg_m = (self.ballista == 1 and 1.25) or (self.ballista == 2 and 1.4) or (self.ballista == 3 and 1.55)
  end

  if self.chronomancy then
    if table.any(self.classes, function(v) return v == 'mage' end) then
      self.chronomancy_aspd_m = (self.chronomancy == 1 and 1.25) or (self.chronomancy == 2 and 1.35) or (self.chronomancy == 3 and 1.45)
    end
  end
  ...
  if self.leader and self.divine_punishment then
    main.current.t:every(5, function()
      ...
      for _, enemy in ipairs(enemies) do
        enemy:hit(enemy.max_hp*6*#mages)
  ...
  if self.reinforce then
    main.current.t:after(0.1, function()
      ...
      if any_enchanter then
        local v = (self.reinforce == 1 and 1.08) or (self.reinforce == 2 and 1.16) or (self.reinforce == 3 and 1.24) or 1
  ...
  if self.leader and self.shoot_5 then
    main.current.t:after(0.1, function()
      main.current.t:every(0.2, function()
        local units = main.current.player:get_all_units()
        local unit = units[5]
        if unit then
          local target = unit:get_closest_object_in_shape(Circle(unit.x, unit.y, 128), main.current.enemies)
          if target then
  ...
  if self.leader then
    self.t:after(1, function()
      ...
      if self.psyker_orbs then
        for i = 1, ((self.psyker_orbs == 1 and 1) or (self.psyker_orbs == 2 and 3) or (self.psyker_orbs == 3 and 5) or 0) do
          local unit = random:table(#psykers > 0 and psykers or units)
          Projectile{group = main.current.main, x = unit.x + 24*math.cos(unit.r), y = unit.y + 24*math.sin(unit.r), color = fg[0], v = 200, dmg = unit.dmg, character = 'psyker', parent = unit}
        end
      end
  ...
  if (self.character == 'psylander') then
	self.psylander_warrior_def_m = 1
	self.psylander_enchanter_dmg_m = 1
	self.psylander_rogue_mvspd_m = 1
	self.psylander_nuker_area_dmg_m = 1
	...
	self.psylander_nuker_bonus = 1
	self.psylander_rogue_bonus = 1
	local all_units = main.current.player:get_all_units()
	for _, unit in ipairs(all_units) do
		  if self.level == 3 and table.any(unit.classes, function(v) return v == 'warrior' end) and unit.character ~= 'psylander' then
				if (self.executions < 6) then
					self.executions = self.executions + 1
...
function Player:update(dt)
  ...
  if self.character == 'squire' then
    ...
    for _, unit in ipairs(all_units) do
      ...
      if self.level == 3 then
        unit.squire_dmg_m = 1.3
        unit.squire_def_m = 1.3
    ...
    local all_units = self:get_all_units()
    for _, unit in ipairs(all_units) do
      if table.any(unit.classes, function(v) return v == 'mercenary' end) then
        psybanker_hp = psybanker_hp + 0.25
      end
    end
  elseif self.character == 'psybanker' then
    local all_units = self:get_all_units()
    for _, unit in ipairs(all_units) do
      if self.level == 3 and table.any(unit.classes, function(v) return v == 'mercenary' end) then
        unit.psybanker_def_m = 1 + (psybanker_hp or 0)
      end
    end
  elseif self.character == 'chronomancer' then
    local all_units = self:get_all_units()
  ...
  if self.character == 'vagrant' and self.level == 3 then
    ...
    self.vagrant_dmg_m = 1 + 0.15*number_of_active_sets
    self.vagrant_aspd_m = 1 + 0.15*number_of_active_sets
  end

  if self.character == 'swordsman' and self.level == 3 then
    self.swordsman_dmg_m = 3
  end
  ...
  if self.character == 'adventurer'  then
    if (self.level == 3) then
		self.druid_stats_m = 1.5
	end
  end

  if self.character == 'mortar'  then
    if (self.level == 3) then
		self.hecatia_damage_m = 1.3
	end
  end

  if self.character == 'brawler' and self.level == 3 then
    local class_levels = get_class_levels(self:get_all_units())
    local number_of_active_sets = 0
    if class_levels.ranger >= 1 then number_of_active_sets = number_of_active_sets + 1 end
    if class_levels.warrior >= 1 then number_of_active_sets = number_of_active_sets + 1 end
    ...
    if class_levels.mercenary >= 1 then number_of_active_sets = number_of_active_sets + 1 end
    if class_levels.explorer >= 1 then number_of_active_sets = number_of_active_sets + 1 end
    self.brawler_def_m = 1 + 0.15*number_of_active_sets
  end
  ...
  if table.any(self.classes, function(v) return v == 'cultist' end) then
    if main.current.cultist_level == 3 then self.cultist_stat_mult = 15
  ...
  if table.any(self.classes, function(v) return v == 'rogue' end) then
    ...
    elseif main.current.rogue_level == 1 then self.chance_to_crit = 15 end
  ...
  if self.blessing then self.heal_effect_m = self.heal_effect_m*((self.blessing == 1 and 1.15) or (self.blessing == 2 and 1.25) or (self.blessing == 3 and 1.35)) end
  ...
  if table.any(self.classes, function(v) return v == 'voider' end) then
    if main.current.voider_level == 2 then self.dot_dmg_m = 1.5
    elseif main.current.voider_level == 1 then self.dot_dmg_m = 1.25
    else self.dot_dmg_m = 1 end
  end
  if self.call_of_the_void then self.dot_dmg_m = (self.dot_dmg_m or 1)*((self.call_of_the_void == 1 and 1.5) or (self.call_of_the_void == 2 and 2) or (self.call_of_the_void == 3 and 2.5) or 1) end
  ...
  if self.ouroboros_technique_l and self.leader then
    ...
    if (state.mouse_control and table.all(self.mouse_control_v_buffer, function(v) return v <= -0.5 end)) or (self.move_left_pressed and love.timer.getTime() - self.move_left_pressed > 1) then
      for _, unit in ipairs(units) do
        unit.ouroboros_def_m = (self.ouroboros_technique_l == 1 and 1.50) or (self.ouroboros_technique_l == 2 and 1.75) or (self.ouroboros_technique_l == 3 and 2)
      end
    else
      for _, unit in ipairs(units) do
  ...
  if self.berserking and table.any(self.classes, function(v) return v == 'warrior' end) then
    self.berserking_aspd_m = math.remap(self.hp/self.max_hp, 0, 1, (self.berserking == 1 and 2) or (self.berserking == 2 and 3) or (self.berserking == 3 and 4), 1)
  end

  if self.speed_3 and self.follower_index == 2 then
    self.speed_3_aspd_m = 1.6
  end

  if self.damage_4 and self.follower_index == 3 then
    self.damage_4_dmg_m = 1.6
  end

  if self.defensive_stance and self.leader then
    self.defensive_stance_def_m = (self.defensive_stance == 1 and 1.2) or (self.defensive_stance == 2 and 1.4) or (self.defensive_stance == 3 and 1.6)
  end
  ...
  if self.offensive_stance and self.leader then
    self.offensive_stance_dmg_m = (self.offensive_stance == 1 and 1.2) or (self.offensive_stance == 2 and 1.4) or (self.offensive_stance == 3 and 1.6)
  end
  ...
  if self.leader and #self.followers == 0 and self.last_stand then
    self.last_stand_dmg_m = 1.25
    self.last_stand_def_m = 1.25
    self.last_stand_aspd_m = 1.25
    self.last_stand_area_size_m = 1.25
    self.last_stand_area_dmg_m = 1.25
    self.last_stand_mvspd_m = 1.25
  end

  if self.dividends and table.any(self.classes, function(v) return v == 'mercenary' end) then
    self.dividends_dmg_m = (1 + gold/100)
    if self.dividends_dmg_m >= 1000 then
      self.dividends_dmg_m = 1000
    end
  end

  if self.character == 'flagellant' and self.level == 3 then
    self.flagellant_hp_m = 5
  end
  ...
  if (self.character == 'psylander') then
	local all_units = self:get_all_units()
	for _, unit in ipairs(all_units) do
		  if self.level == 3 and table.any(unit.classes, function(v) return v == 'warrior' end) and unit.character ~= 'psylander' and (player) then
		  if self.level == 3 and table.any(unit.classes, function(v) return v == 'warrior' end) and unit.character ~= 'psylander' then
				if (self.executed < 6) then
					self.executed = self.executed + 1
					unit:hit(99999999)
				end
				if (self) then self:recalculate_followers() end
				if (unit.leader) then unit:recalculate_followers() end
		  end
       end
	end
  end
  ...
  self.buff_aspd_m = self.buff_aspd_m*(self.cultist_mult or 1)
  ...
  self.buff_dmg_m = self.buff_dmg_m*(self.ritual_dmg_m or 1)*(self.init_ritual_dmg_m or 1)*(self.cultist_mult or 1)
  ...
  self.buff_def_m = self.buff_def_m*(self.cultist_mult or 1)
  ...
  self.buff_area_size_m = self.buff_area_size_m*(self.cultist_mult or 1)
  ...
  self.buff_area_dmg_m = self.buff_area_dmg_m*(self.cultist_mult or 1)
  ...
  self.buff_mvspd_m = self.buff_mvspd_m*(self.cultist_mult or 1)
...
function Player:hit(damage, from_undead)
  ...
  if self.payback and table.any(self.classes, function(v) return v == 'enchanter' end) then
    ...
    for _, unit in ipairs(units) do
      ...
      unit.payback_dmg_m = unit.payback_dmg_m + ((self.payback == 1 and 0.05) or (self.payback == 2 and 0.1) or (self.payback == 3 and 0.15) or 0)
    end
  end
  ...
  if self.character == 'psykeeper' then
    ...
    if self.level == 3 then
      local enemies = main.current.main:get_objects_by_classes(main.current.enemies)
      for _, enemy in ipairs(enemies) do
        enemy:hit(3*actual_damage/#enemies)
      end
    end
  end
  ...
  if self.hp <= 0 then
    ...
    elseif self.lasting_7 and self.follower_index == 6 and not self.undead then
      self.undead = true
      self.t:after(20, function() self:hit(10000, true) end)
      ...
      if self.leader and #self.followers == 0 then
        ...
        main.current.unitsDead = main.current.unitsDead + 1
      end

	  if self.sacrifice then
		local units = self:get_all_units()
		for _, unit in ipairs(units) do
		  if table.any(unit.classes, function(v) return v == 'cultist' end) then
			unit:heal(unit.max_hp*(self.sacrifice == 1 and 0.25) or (self.sacrifice == 2 and 0.50) or (self.sacrifice == 3 and 0.75))
		  end
		end
      end
      ...
      if self.kinetic_bomb then
        ...
        for _, enemy in ipairs(enemies) do
          enemy:push(random:float(120, 200)*(self.knockback_m or 1), self:angle_to_object(enemy))
        end
      end

       local unit = self:get_unit('necromancer')
	  
	  if unit and unit.level == 3 then
		if not (unit.triggered_necromancer) then
		  Critter{group = main.current.main, x = self.x, y = self.y, color = dblue[0], r = random:float(0, 2*math.pi), v = 10, dmg = self.dmg, parent = self}
		end
        unit.triggered_necromancer = love.timer.getTime()
        unit.t:after(1, function() unit.triggered_necromancer = false end, 'haste')
        end
      ...
      if self.porcupine_technique then
        main.current.t:after(0.01, function()
          ...
          for i = 1, 16 do
            ...
            HitCircle{group = main.current.effects, x = self.x + 0.8*self.shape.w*math.cos(r), y = self.y + 0.8*self.shape.w*math.sin(r), rs = 60}
            local t = {group = main.current.main, x = self.x + 1.6*self.shape.w*math.cos(r), y = self.y + 1.6*self.shape.w*math.sin(r), v = 250, r = r, color = self.color, dmg = self.dmg*5,
            parent = self, character = 'barrage', level = self.level, pierce = 1000, ricochet = 5}
      ...
      if self.annihilation and table.any(self.classes, function(v) return v == 'voider' end) then
        ...
        for _, enemy in ipairs(enemies) do
          enemy:apply_dot(self.dmg*(self.dot_dmg_m or 1)*(main.current.chronomancer_dot or 1), 10)
        end
      end

      if self.insurance then
        if random:bool(6*((main.current.mercenary_level == 2 and 16) or (main.current.mercenary_level == 1 and 8) or 0)) then
          main.current.t:after(0.01, function()
...
function Player:sorcerer_repeat()
  ...
  if enemy then
    if self.burning_field then
      ...
      DotArea{group = main.current.effects, x = enemy.x, y = enemy.y, rs = self.area_size_m*48, color = red[0], dmg = 90*self.area_dmg_m*(self.dot_dmg_m or 1), duration = 4, character = 'burning_field'}
    end
  end
  ...
  if enemy then
    if self.freezing_field then
      ...
      Area{group = main.current.effects, x = enemy.x, y = enemy.y, w = self.area_size_m*54, color = blue[0], character = 'freezing_field', parent = self}
    end
  end
end

function Player:cult_attack()
	if not self.ritual_dmg_m then
		self.ritual_dmg_m = 1
	end
	if self.ritual then
	self.ritual_dmg_m = self.ritual_dmg_m + 0.01
	end
end
...
function Projectile:init(args)
  ...
  if self.character == 'sage' then
    ...
    self.dmg = 0
    self.pull_sensor = Circle(self.x, self.y, 96*self.parent.area_size_m)
    self.rs = 0
    ...
    self.t:after(4, function()
      self.t:every_immediate(0.05, function() self.hidden = not self.hidden end, 7, function()
        ...
        if self.level == 3 then
          ...
          for _, enemy in ipairs(enemies) do
            enemy:hit(9*self.compression_dmg)
          end
        end
      end)
    end)
  ...
  elseif self.character == 'psyker' then
    ...
    self.orbit_speed = random:float(2, 4)*((self.parent.orbitism == 1 and 1.33) or (self.parent.orbitism == 2 and 1.66) or (self.parent.orbitism == 3 and 1.99) or 1)*(1/self.parent.aspd_m)
    self.orbit_offset = random:float(0, 2*math.pi)
    self.dmg = self.dmg*((self.parent.psychosink == 1 and 1.5) or (self.parent.psychosink == 2 and 2) or (self.parent.psychosink == 3 and 2.5) or 1)
  ...
  elseif self.character == 'arcanist' then
    ...
    self.t:every(self.parent.level == 3 and 0.54 or 0.8, function()
	 ...
	 self.t:after(1, function()
		self.t:every(0.05, function()
			local r = self:angle_to_object(self.parent)
			self.r = r
			self.v = self.v + 1
		end)
	 end)
  ...
  elseif self.character == 'witch' and self.level == 3 then
    ...
    elseif self.character == 'blood_elf' then
    self.t:after(1, function() 
  ...
  if self.parent.divine_machine_arrow and table.any(self.parent.classes, function(v) return v == 'ranger' end) then
    if random:bool((self.parent.divine_machine_arrow == 1 and 15) or (self.parent.divine_machine_arrow == 2 and 25) or (self.parent.divine_machine_arrow == 3 and 35)) then
      self.homing = true
  ...
  if self.parent.blunt_arrow and table.any(self.parent.classes, function(v) return v == 'ranger' end) then
    if random:bool((self.parent.blunt_arrow == 1 and 15) or (self.parent.blunt_arrow == 2 and 30) or (self.parent.blunt_arrow == 3 and 45)) then
      self.knockback = 40
    end
  end

  if self.parent.flying_daggers and table.any(self.parent.classes, function(v) return v == 'rogue' end) then
    self.chain = self.chain + ((self.parent.flying_daggers == 1 and 1) or (self.parent.flying_daggers == 2 and 2) or (self.parent.flying_daggers == 3 and 3))
  end
end
...
function Projectile:draw()
  ...
  elseif self.character == 'arcanist' then
    ...
    elseif self.character == 'boomerang' then
    graphics.push(self.x, self.y, self.r+self.rx, self.hfx.hit.x, self.hfx.hit.x)
      graphics.rectangle(self.x, self.y, self.shape.w, self.shape.h, 2, 2, self.color)
      graphics.rectangle(self.x, self.y, self.shape.h, self.shape.w, 2, 2, self.color)
    graphics.pop()
...
function Projectile:on_collision_enter(other, contact)
  ...
  if other:is(Wall) then
    if self.character == 'archer' or self.character == 'hunter' or self.character == 'barrage' or self.character == 'barrager' or self.character == 'sentry' or self.character == 'sniper' or self.character == 'blood_elf' then
    ...
    elseif self.character == 'artificer_death' or self.character == 'boomerang' then
      if self.ricochet <= 0 then
    ...
    elseif self.character == 'wizard' or self.character == 'lich' or self.character == 'arcanist' or self.character == 'arcanist_projectile' or self.character == 'witch' then
      self:die(x, y, r, random:int(2, 3))
      magic_area1:play{pitch = random:float(0.95, 1.05), volume = 0.075}
       elseif self.character == 'rocketeer' then
      self:die(x, y, r, random:int(2, 3))
      explosion1:play{pitch = random:float(0.95, 1.05), volume = 0.1}
    elseif self.character == 'cannoneer' then
      self:die(x, y, r, random:int(2, 3))
...
function Projectile:on_trigger_enter(other, contact)
  ...
  if self.character == 'boomerang' and other == self.parent then
	   self:die()
	end
  ...
  if table.any(main.current.enemies, function(v) return other:is(v) end) then
      ...
      if self.chain > 0 then
        ...
        if object then
          ...
          if self.parent.ultimatum then
            self.dmg = self.dmg*((self.parent.ultimatum == 1 and 1.15) or (self.parent.ultimatum == 2 and 1.3) or (self.parent.ultimatum == 3 and 1.45))
          end
        end
      end
      HitCircle{group = main.current.effects, x = self.x, y = self.y, rs = 6, color = fg[0], duration = 0.1}
    ...
    if self.character == 'rocketeer' then
		if (self.level == 3) then
			Area{group = main.current.effects, x = self.x, y = self.y, r = self.r, w = self.parent.area_size_m*120, color = self.color, dmg = self.parent.area_dmg_m*self.dmg, character = self.character, parent = self,
        void_rift = self.parent.void_rift, echo_barrage = self.parent.echo_barrage}
		else
			Area{group = main.current.effects, x = self.x, y = self.y, r = self.r, w = self.parent.area_size_m*60, color = self.color, dmg = self.parent.area_dmg_m*self.dmg, character = self.character, parent = self,
        void_rift = self.parent.void_rift, echo_barrage = self.parent.echo_barrage}
		end
	end
    ...
    if self.parent and self.parent.chain_infused then
      ...
      local src = other
      for i = 1, 4 + (stormweaver_level == 3 and 4 or 0) do
        _G[random:table{'spark1', 'spark2', 'spark3'}]:play{pitch = random:float(0.9, 1.1), volume = 0.3}
        ...
        if dst then
          dst:hit(0.4*self.dmg*(self.distance_dmg_m or 1))
        ...
        for j = 1, 3 do
          main.current.t:after((j-1)*0.1, function()
            ...
            for i = 1, 3 do
              ...
              if dst then
                dst:hit(0.5*((self.parent.lightning_strike == 1 and 0.6) or (self.parent.lightning_strike == 2 and 0.8) or (self.parent.lightning_strike == 3 and 1))*self.dmg*(self.distance_dmg_m or 1))
                LightningLine{group = main.current.effects, src = src, dst = dst}
                src = dst 
              end
            end
...
function Area:init(args)
  ...
  for _, enemy in ipairs(enemies) do
    local resonance_dmg = 0
    local resonance_m = (self.parent.resonance == 1 and 0.03) or (self.parent.resonance == 2 and 0.06) or (self.parent.resonance == 3 and 0.10) or 0
    if self.character == 'elementor' then
      if self.parent.resonance then resonance_dmg = 2*self.dmg*resonance_m*#enemies end
      enemy:hit(3*self.dmg + resonance_dmg, self)
      if self.level == 3 then
        enemy:slow(0.4, 6)
      end
      elseif self.character == 'sylph' then
    ...
    elseif self.character == 'launcher' then
      ...
      elseif self.character == 'locust' then
	  Critter{group = main.current.main, x = self.x+random:float(-0.5*self.w, 0.5*self.w), y = self.y+random:float(-0.5*self.w, 0.5*self.w), color = orange[0], r = random:float(0, 2*math.pi), v = 0, dmg = self.dmg, parent = self.parent}
    elseif self.character == 'freezing_field' then
      enemy:slow(0.25, 4)
    elseif self.parent:is(Critter) then
     enemy:hit(self.dmg * enemy.max_hp)
    else
...
function DotArea:init(args)
  ...
  if self.character == 'plague_doctor' or self.character == 'pyromancer' or self.character == 'burning_field' then
    self.t:every(0.2, function()
      local enemies = main.current.main:get_objects_in_shape(self.shape, main.current.enemies)
      if self.character == 'mortar' then
			if self.level == 3 then
				self.rs = self.rs + 0.25
				self.shape = Circle(self.x, self.y, self.rs)
			end
	  end
      ...
      for _, enemy in ipairs(enemies) do
        ...
        if (self.character == 'acolyte_dot' and self.level == 3) then
			enemy:curse('dagger', 0.2*(self.hex_duration_m or 1), 1, 1, 1)
		end
  ...
  elseif self.character == 'psylander' then
    self.t:every(0.2, function()
	  self.psylander_force_power = self.parent.psylander_forcer_power
      local enemies = main.current.main:get_objects_in_shape(self.shape, main.current.enemies)
      if #enemies > 0 then
        self.spring:pull(0.15, 200, 10)
        hit2:play{pitch = random:float(0.8, 1.2), volume = 0.4}
      end
      for _, enemy in ipairs(enemies) do
        enemy:hit((self.dot_dmg_m or 1)*self.dmg/1.25, self, true)
        HitCircle{group = main.current.effects, x = enemy.x, y = enemy.y, rs = 6, color = fg[0], duration = 0.1}
        for i = 1, 1 do HitParticle{group = main.current.effects, x = enemy.x, y = enemy.y, color = self.color} end
        for i = 1, 1 do HitParticle{group = main.current.effects, x = enemy.x, y = enemy.y, color = enemy.color} end
      end
  ...
  elseif self.character == 'cryomancer' then
    self.t:every(1, function()
      ...
      if #enemies > 0 then
        self.spring:pull(0.15, 200, 10)
        frost1:play{pitch = random:float(0.8, 1.2), volume = 0.35}
      end
      for _, enemy in ipairs(enemies) do
        if self.level == 3 then
          enemy:slow(0.2, 4)
          enemy:hit((self.dot_dmg_m or 1)*6*self.dmg, self, true)
  ...
  elseif self.void_rift then
    self.t:every(0.2, function()
      ...
      for _, enemy in ipairs(enemies) do
        hit2:play{pitch = random:float(0.8, 1.2), volume = 0.2}
        enemy:hit((self.dot_dmg_m or 1)*self.dmg/2.5, self, true)
        HitCircle{group = main.current.effects, x = enemy.x, y = enemy.y, rs = 6, color = fg[0], duration = 0.1}
        for i = 1, 1 do HitParticle{group = main.current.effects, x = enemy.x, y = enemy.y, color = self.color} end
        for i = 1, 1 do HitParticle{group = main.current.effects, x = enemy.x, y = enemy.y, color = enemy.color} end
      end
...
function DotArea:update(dt)
  ...
  if self.parent then
    if (self.character == 'plague_doctor' and self.level == 3 and not self.plague_doctor_unmovable) or self.character == 'cryomancer' or self.character == 'pyromancer' or (self.character == 'psylander' and not self.psylander_immobile) then
...
function ForceArea:init(args)
  ...
  if self.character == 'psykino' then
    ...
    self.t:after(2 - 0.35, function()
      ...
      if self.level == 3 then
        ...
        local enemies = main.current.main:get_objects_in_shape(self.shape, main.current.enemies)
        for _, enemy in ipairs(enemies) do
          enemy:hit(16*self.parent.dmg)
          enemy:push(10*(self.knockback_m or 1), self:angle_to_object(enemy))
        end
      end
    end)
  ...
  elseif self.character == 'gravity_field' then
    ...
    self.t:tween(3, self, {dvr = 0}, math.linear)

    self.t:during(3, function()
      local enemies = main.current.main:get_objects_in_shape(self.shape, main.current.enemies)
    ...
    self.t:after(3 - 0.35, function()
      local enemies = main.current.main:get_objects_in_shape(self.shape, main.current.enemies)
      self.t:every_immediate(0.05, function() self.hidden = not self.hidden end, 7, function() self.dead = true end)
      for _, enemy in ipairs(enemies) do
        enemy:push(200 * (self.knockback_m or 1), self:angle_to_object(enemy))
      end
    end)
  end
end
  ...
  self.t:after(12*(self.parent.conjurer_buff_m or 1), function()
    self.t:every_immediate(0.05, function() self.hidden = not self.hidden end, 7, function()
      ...
      if self.parent.construct_instability then
        camera:shake(2, 0.5)
        local n = (self.parent.construct_instability == 1 and 2) or (self.parent.construct_instability == 2 and 3) or (self.parent.construct_instability == 3 and 4) or 1
...
function Volcano:init(args)
  ...
  self.t:every(self.level == 3 and 0.333 or 1, function()
    camera:shake(4, 0.5)
    ...
    Area{group = main.current.effects, x = self.x, y = self.y, r = self.r, w = self.parent.area_size_m*90, r = random:float(0, 2*math.pi), color = self.color, dmg = (self.parent.area_dmg_m or 1)*self.parent.dmg*1.5,
...
Shrine = Object:extend()
Shrine:implement(GameObject)
Shrine:implement(Physics)
function Shrine:init(args)
  self:init_game_object(args)
  if not self.group.world then self.dead = true; return end
  if tostring(self.x) == tostring(0/0) or tostring(self.y) == tostring(0/0) then self.dead = true; return end
  self:set_as_rectangle(9, 9, 'static', 'player')
  ...
  fire1:play{pitch = random:float(0.95, 1.05), volume = 0.5}
  self.t:after(0.1, function()
    self.dot_area = DotArea{group = main.current.effects, x = self.x, y = self.y, rs = 64, color = dblue[0], dmg = self.parent.dmg, character = 'acolyte_dot', level = self.level, parent = self}
  end)
...
function Shrine:draw()
  ...
  graphics.push(self.x, self.y, -math.pi/2, self.spring.x, self.spring.x)
    graphics.triangle_equilateral(self.x, self.y, self.shape.w, self.hfx.hit.f and fg[0] or self.color, 3)
    graphics.circle(self.x, self.y, 1.5*self.shape.w, self.hfx.hit.f and fg[0] or self.color, 2)
    graphics.circle(self.x, self.y, 1.5*self.shape.w, self.hfx.hit.f and fg_transparent[0] or self.color_transparent)
  graphics.pop()
end
...
function Sentry:init(args)
  ...
  self.t:after(20*(self.parent.conjurer_buff_m or 1), function()
    local n = n or random:int(3, 4)
    for i = 1, n do HitParticle{group = main.current.effects, x = self.x, y = self.y, r = random:float(0, 2*math.pi), color = self.color} end
    ...
    for j = 1, n do
      self.t:after((j-1)*0.1, function()
        for i = 1, 4 do
          ...
          dmg = self.parent.dmg*(self.parent.conjurer_buff_m or 1), character = 'sentry', parent = self.parent, ricochet = self.parent.level == 3 and 5 or 0}
          Projectile(table.merge(t, mods or {}))
          r = r + math.pi/2
        end
      end)
    ...
    if self.parent.rearm then
      self.t:after(0.25, function()
        ...
        for i = 1, 4 do
          ...
          HitCircle{group = main.current.effects, x = self.x + 0.8*self.shape.w*math.cos(r), y = self.y + 0.8*self.shape.w*math.sin(r), rs = 6}
          local t = {group = main.current.main, x = self.x + 1.6*self.shape.w*math.cos(r), y = self.y + 1.6*self.shape.w*math.sin(r), v = 200, r = r, color = self.color,
          dmg = self.parent.dmg*(self.parent.conjurer_buff_m or 1), character = 'sentry', parent = self.parent, ricochet = self.parent.level == 3 and 3 or 0}
          Projectile(table.merge(t, mods or {}))
          r = r + math.pi/2
        end
...
function Sentry:update(dt)
  ...
  self:set_angle(self.r)
  self.t:set_every_multiplier('attack', self.parent.level == 3 and 0.4 or self.parent.level == 2 and 0.7 or 1)
end
...
function Bomb:init(args)
  ...
  self.color = orange[0]
  self.dmg = 3*get_character_stat('bomber', self.level, 'dmg')
...
function Bomb:explode()
  camera:shake(4, 0.5)
  local t = {group = main.current.effects, x = self.x, y = self.y, r = self.r, w = self.parent.area_size_m*64*(self.level == 3 and 3 or 1), color = self.color, 
    dmg = self.parent.area_dmg_m*self.dmg*(self.parent.conjurer_buff_m or 1)*(self.level == 3 and 3 or 1), character = self.character, parent = self.parent}
  ...
  self.t:after(0.25, function()
    if self.parent.construct_instability then
      camera:shake(2, 0.5)
      local n = (self.parent.construct_instability == 1 and 2) or (self.parent.construct_instability == 2 and 3) or (self.parent.construct_instability == 3 and 4) or 1
      Area{group = main.current.effects, x = self.x, y = self.y, r = self.r + random:float(-math.pi/16, math.pi/16), w = self.parent.area_size_m*48*(self.level == 3 and 3 or 1), color = self.color, 
        dmg = n*self.parent.dmg*self.parent.area_dmg_m*(self.level == 3 and 3 or 1), parent = self.parent}
      _G[random:table{'cannoneer1', 'cannoneer2'}]:play{pitch = random:float(0.95, 1.05), volume = 0.5}
      self.dead = true
    end
...
function Automaton:update(dt)
  ...
  self.t:set_every_multiplier('shoot', self.parent.level == 3 and 0.5 or 1)
...
function HealingOrb:init(args)
  ...
  if main.current.healer_level > 0 and not self.healer_effect_orb then
    if random:bool((main.current.healer_level == 1 and 20) or (main.current.healer_level == 2 and 40)) then
...
function HealingOrb:on_trigger_enter(other, contact)
  ...
  if other:is(Player) then
    ...
    if main.current.player.divine_barrage and random:bool((main.current.player.divine_barrage == 1 and 30) or (main.current.player.divine_barrage == 2 and 60) or (main.current.player.divine_barrage == 3 and 90)) then
      trigger:after(0.01, function()
        if not main.current.main.world then return end
...
function Critter:die(x, y, r, n)
  ...
  if main.current.player.baneling_burst then
    ...
    Area{group = main.current.effects, x = self.x, y = self.y, r = self.r, w = self.parent.area_size_m*24, color = self.color,
      dmg = (main.current.player.baneling_burst == 1 and 2*self.hp) or (main.current.player.baneling_burst == 2 and 3*self.hp) or (main.current.player.baneling_burst == 3 and 4*self.hp) or 0, parent = self.parent}
    _G[random:table{'cannoneer1', 'cannoneer2'}]:play{pitch = random:float(0.95, 1.05), volume = 0.5}
  end
end
